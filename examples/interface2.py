...
ss.variables["rhoA"].data += 10.0e1 * (
    1.0 + 0.1 * numpy.exp(-((x - numpy.pi * 1.5) ** 2) / 0.01)
)

# Init momenta
ss.variables["rhou"].data = ss.variables["rho"].data * 0.70
ss.variables["rhoAuA"].data = ss.variables["rhoA"].data * 0.70
ss.variables["u1"].data = 0.70
