...
class pyrandaLEOS(pyrandaPackage):
    ...
    def addMaterial(
        self, EOS_number, function_names=["Pt", "Et", "Cs"], matID=None, dxdy=False
    ):
        ...
        self.materials[matID] = self.__setupLEOS__(
            EOS_number, function_names, dxdy=dxdy
        )
...
class leosFunc:
    ...
    def ieval(self, rho, E, Tg=None, N=None):
        ...
        T, dx, dy = self.eosFunc.inverseEval(
            int(N), rho.tolist(), E.tolist(), self.lopts
        )
        return numpy.array(T)
