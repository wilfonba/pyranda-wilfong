...
class ipoint:
    ...
    def interpFast(self, vals):
        ...
        vals = numpy.einsum(
            "nj,nj->n", numpy.take(vlocal.flatten(), self.vtx), self.SIweight
        )
...
def getPoints(x, y, f, pts, imethod):
    ...
    dc = SI.griddata(
        numpy.array([xx, yy]).T, ff, numpy.array([xpts, ypts]).T, method=imethod
    )
